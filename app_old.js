...
function initApp(){
  try{
    ...
    console.error(err);
    setError("Erro ao inicializar app.js: " + (err.message || err));
  }
}
...
function bindImport(){
  ...
  if(dropzone && fileInput){
    ...
    dropzone.addEventListener("drop", e => {
      ...
      setStatus("Arquivo arrastado. Clique em Processar e padronizar.");
    });
  }
}
...
async function processFile(){
  try{
    ...
    if(!BASE_RAW.length){
      throw new Error("Nenhuma linha válida encontrada. Verifique se o extrato tem Data na coluna A, Lançamento na coluna B e Valor na coluna E.");
    }
...
async function readFileToRows(file){
  ...
  if(typeof XLSX === "undefined"){
    throw new Error("Biblioteca XLSX não carregou. Confira a conexão com internet e o CDN do SheetJS.");
  }
...
function buildDenomSummary(raw){
  ...
  raw.forEach(r => {
    ...
    if(!map.has(d)){
      map.set(d,{denom:d,count:0,total:0,suggested:isSuggestedTariff(d)});
    }
    const obj = map.get(d);
    obj.count += 1;
    obj.total += r.Valor;
...
function renderDenomSelector(){
  ...
  if($("selectionInfo")){
    $("selectionInfo").textContent = `${SELECTED_DENOMS.size.toLocaleString("pt-BR")} selecionadas de ${DENOM_SUMMARY.length.toLocaleString("pt-BR")}`;
  }
...
function renderAnalysis(){
  ...
  const ranking = buildDenomSummary(rows).sort((a,b) => b.total-a.total).slice(0,15);
...
function renderChartTop(ranking){
  ...
  chartTop = new Chart($("chartTop"), {
    ...
    data:{
      ...
      datasets:[{label:"Total",data:ranking.map(x => x.total),backgroundColor:"rgba(46,125,50,.45)",borderColor:"#2e7d32",borderWidth:1}]
    },
...
function renderRanking(ranking){
  if($("rankingTable")){
    $("rankingTable").innerHTML = tableHtml(
      ranking.map(x => ({"Denominação":x.denom,"Qtd":x.count,"Total":round2(x.total)})),
...
function renderTables(){
  if($("rawTable")) $("rawTable").innerHTML = tableHtml(BASE_RAW.slice(0,150), ["Data","Unidade","Banco","Denominação","Valor","Mes","Tipo"]);
...
function exportWorkbook(){
  ...
  XLSX.writeFile(wb, "base_itau_tarifas_gt.xlsx");
}
...
function buildPivot(rows){
  ...
  return [...map.entries()].sort((a,b)=>a[0].localeCompare(b[0],"pt-BR")).map(([d,values]) => {
...
function resetAll(){
  ...
  if($("rawTable")) $("rawTable").innerHTML = "";
